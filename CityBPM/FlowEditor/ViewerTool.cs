...
namespace FlowEditor
{
    ...
    public class AddNodeTool : ViewerTool
    {
        ...
        {
            Node preNode = MainPage.Current.nodes.Last().Key;
            MainPage.Current.MyCanvas.Children.Remove(preNode);
            MainPage.Current.nodes.Remove(preNode);
        }
    ...
    public class AddConnectionTool : ViewerTool
    {
        ...
        public override void MouseLDownHandler(object sender, MouseButtonEventArgs e)
        {
            ...
            if (_isTbAdd)
            {
                MainPage.Current.MyCanvas.Children.Remove(border);
                _isTbAdd = false;
            }
            ...
            _mouseDownTemp = e.GetPosition(MainPage.Current.MyCanvas);
    ...
    public class MoveNodeTool : ViewerTool
    {
        ...
        {
            _isDragging = true;
            _mouseDownTemp = GetCatchPoint(e.GetPosition(MainPage.Current.MyCanvas));
            foreach (var child in MainPage.Current.MyCanvas.Children)
            {
                if (child is Node)
                {
                    Node node = child as Node;
                    if (node.IsPointInNode(e.GetPosition(MainPage.Current.MyCanvas)))
                    {
                        _movingNode = node;
                        _isPickedAny = true;
        ...
        {
            int index = MainPage.Current.nodes.Keys.ToList().IndexOf(_movingNode);
            ...
            MainPage.Current.connections.ForEach(c =>
            {
                if (c.Key.from == index)
                {
        ...
        public override void MouseLDownHandler(object sender, MouseButtonEventArgs e)
        {
            ...
            foreach (var child in MainPage.Current.MyCanvas.Children)
            {
                if (child is Node)
                {
                    ...
                    {
                        if (MainPage.Current.CurrentNode != null)
                        {
                            MainPage.Current.CurrentNode.SetColor(_defaultColor);
                        }
                        MainPage.Current.CurrentNode = node;
    ...
    public class EditNodeTool : ViewerTool
    {
        ...
        void OKButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            int s = 1;
            while (MainPage.Current.nodes.Keys.Where(n=>n!=currentNode).ToList().Any(n => n.Text == name))
            {
    ...
    public class EditConnectionTool : ViewerTool
    {
        ...
        public override void MouseLUpHandler(object sender, MouseButtonEventArgs e)
        {
            if (count == 2)
            {
                ...
                if (_startNode != null && _endNode != null)
                {
                    int start = MainPage.Current.nodes.Keys.ToList().IndexOf(_startNode);
                    int end = MainPage.Current.nodes.Keys.ToList().IndexOf(_endNode);
                    if (MainPage.Current.connections.Keys.Any(x=>x.from == start))
                    {
                        MainPage.Current.connections.Keys.Where(x => x.from == start).ForEach(x =>
                        {
                            if (x.to == end)
                            {
                                _currentConn = x;
        ...
        void OKButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            //MainPage.Current.connections[_currentConn].LabelText = connectionInfo;
            MainPage.Current.connections[_currentConn].ReadyControl();
        }
