// TongJi.Geometry.cs for Silverlight
...
namespace TongJi.Geometry
{
    ...
    public class Point2DString
    {
        ...
        private Point2D GetIntersection(Point2D p0, Point2D p1, Point2D d0, Point2D d1)
        {
            ...
            if (sqrKross > 0.0001 * sqrLen0 * sqrLen1)
            {
                //Point2D pt = new Point2D((p0.x + p1.x) / 2, (p0.y + p1.y) / 2);
                return new Point2D();
            }
            //lines are the same
            else
        ...
        //        return;
        //    }
        //    for (int count = 0; count < points.Count - 1; )
        //    {
        //        if (points[count].DistTo(points[count + 1]) < limitDist)
        //        {
