...
namespace UPlanWeb.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            if (UserManager.GetDbRecord(model.UserName).IsForbidden == true)
            {
                ModelState.AddModelError("", "抱歉，您的账户已被封禁，无法登录。");
                return View(model);
            }
            if (ModelState.IsValid && WebSecurity.Login(model.UserName, model.Password, persistCookie: model.RememberMe))
            {
                TongJi.Web.Notifications.Notification.EnqueueMessage(model.UserName, string.Format("尊敬的用户 {0}，欢迎您登录本系统。", model.UserName));
                TongJi.Web.Notifications.Notification.EnqueueMessage(model.UserName, "请查看您的待办事项和站内信。");
        ...
        {
            WebSecurity.Logout();
            TongJi.Web.Notifications.Notification.EnqueueMessage("", "您的账户已从系统退出。");
        ...
        public ActionResult ExternalLoginConfirmation(RegisterExternalLoginModel model, string returnUrl)
        {
            ...
            {
                // 将新用户插入到数据库
                using (var db = new BasicWebContext())
                {
